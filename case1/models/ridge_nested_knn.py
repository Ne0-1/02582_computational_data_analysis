...
for i, (outer_train_index, outer_test_index) in enumerate(kf1.split(X)):
    ...
    for j, (inner_train_index, inner_test_index) in enumerate(kf2.split(X_par)):
        ...
        imputer_train = KNNImputer(n_neighbors=5, weights='distance', metric='nan_euclidean')
    ...
    imputer_par = KNNImputer(n_neighbors=5, weights='distance', metric='nan_euclidean')
    imputer_par.fit(X_par[:, :95])
    X_par[:, :95] = imputer_par.transform(X_par[:, :95])
    X_test[:, :95] = imputer_par.transform(X_test[:, :95])
...
plt.savefig('coeffs_ridge_nested_knn.png', dpi=100)
plt.show()
