...
for i, (outer_train_index, outer_test_index) in tqdm(enumerate(kf1.split(X, y))):
    ...
    for j, (inner_train_index, inner_test_index) in enumerate(kf2.split(X_par)):
        ...
        # Real valued features (mean)
        X_train[:, :95] = np.where(np.isnan(X_train[:, :95]), np.ma.array(X_train[:, :95], mask=np.isnan(X_train[:, :95])).mean(axis=0), X_train[:, :95])
        X_val[:, :95] = np.where(np.isnan(X_val[:, :95]), np.ma.array(X_val[:, :95], mask=np.isnan(X_val[:, :95])).mean(axis=0), X_val[:, :95])

        # Categorical (mode)
        imputer_train = SimpleImputer(strategy='most_frequent')
        imputer_train.fit(X_train)
        X_train = imputer_train.transform(X_train)
        X_val = imputer_train.transform(X_val)
    ...
    X_par[:, :95] = np.where(np.isnan(X_par[:, :95]), np.ma.array(X_par[:, :95], mask=np.isnan(X_par[:, :95])).mean(axis=0), X_par[:, :95])
    X_test[:, :95] = np.where(np.isnan(X_test[:, :95]), np.ma.array(X_test[:, :95], mask=np.isnan(X_test[:, :95])).mean(axis=0), X_test[:, :95])

    # Categorical (mode)
    imputer_par = SimpleImputer(strategy='most_frequent')
    imputer_par.fit(X_par)
    X_par = imputer_par.transform(X_par)
    X_test = imputer_par.transform(X_test)
...
plt.savefig('coeffs_elasticnet_nested_freq.png', dpi=100)
plt.show()
